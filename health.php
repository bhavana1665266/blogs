...
<body>
   ...
   <?php
            ...
            $sql = "SELECT title FROM post WHERE domain='health'";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    // echo '<li><a href="?title=' . urlencode($row["title"]) . '"style="text-decoration: none;">' . $row["title"] . '</a></li>';
                    echo '<div class="message-box">
                    <div class="message">';
